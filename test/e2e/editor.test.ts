...
describe.each(ENTRY_POINTS)("two server processes, one host (%s)", (_label, SERVER) => {
  ...
  async function spawnServer(forRoot: string): Promise<Client> {
  ...
  beforeAll(async () => {
    ...
    model = await spawnServer(root);
    panel = await spawnServer(root);
  });
  ...
  it("lets the panel claim a proposal the other process opened", async () => {
    ...
    await call(model, "propose_write", { path: "claim-me.txt", content: "hello\n" });

    // Act: the panel asks the other one, with only the path it was handed.
    const claimed = await call(panel, "editor_pending", { path: "claim-me.txt" });
  ...
  it("commits a proposal the other process opened", async () => {
    // Arrange.
    const opened = await call(model, "propose_write", {
    ...
    const attached = await call(panel, "editor_attach", { proposalId });
    const committed = await call(panel, "editor_commit", { proposalId });
  ...
  it("writes an approved proposal once, however many processes are asked", async () => {
    ...
    const { proposalId } = opened.structuredContent as unknown as ProposalHandle;
    await call(panel, "editor_attach", { proposalId });
    await call(panel, "editor_commit", { proposalId });

    // Act: the other process is asked to write the same approval again.
    const second = await call(model, "editor_commit", { proposalId });
    ...
    expect(said(second)).toMatch(/already committed/i);
  ...
  it("keeps servers with different roots apart", async () => {
    ...
    const stranger = await spawnServer(elsewhere);

    try {
      // Act.
      await call(model, "propose_write", { path: "not-yours.txt", content: "x\n" });
      const claimed = await call(stranger, "editor_pending", { path: "not-yours.txt" });
